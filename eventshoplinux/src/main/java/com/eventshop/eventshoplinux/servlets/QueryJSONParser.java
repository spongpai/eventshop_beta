...
//	}
	...
	public QueryProcess parseQuery(int qMasterId, List<String> queryTree, FrameParameters finalFrame)
	{
		...
		JsonArray queryArr = (JsonArray) parser.parse(queryTree.get(0)); // have to first get the query Array --sanjukta 06-08-2014			
		
		for(int i = 0; i < queryArr.size(); i++)
		{
			JsonObject query = queryArr.get(i).getAsJsonObject(); // then get the first element and convert to jsonobject --sanjukta 06-08-2014
	...
	private String parseGrouping(JsonObject query)
	{
		...
		if(method.equalsIgnoreCase("KMeans"))
		{
			...
			groupCC += (split + ",false");		// Note: 141208 By Siripen
												// change the doColoring flag to "false"... 
												// if the doColoring is true, the value in the json emage will be the color value
												// We need to keep category number (1,2,3) instead of actual color value
